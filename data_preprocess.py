...
training_data_line_path = os.path.join('data', 'movie_lines.txt')
training_data_conversation_path = os.path.join('data', 'movie_conversations.txt')

def main(args):
    max_data = args.max
    logging.info('Process Training Data...')
    line_df = pd.read_csv(training_data_line_path, sep=" \+\+\+\$\+\+\+ ", names=['lineID', 'characterID', 'movieID', 'character', 'utterance'], engine='python')
